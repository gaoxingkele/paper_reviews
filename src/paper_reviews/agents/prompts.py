...
仅当存在**致命且不可在评审中修复**的问题时才判 pass=false（桌面退稿）：明显超出期刊范围、
抄袭/数据造假/一稿多投等诚信硬伤、稿件严重不完整（缺方法或结果、无法评审）。

注意：**可轻易补正的遗漏不构成桌面退稿**（pass 仍为 true，仅在 red_flags 提示）——例如：
未写 AI 使用声明、缺数据可用性声明、格式/模板小问题、引用不全、伦理声明缺失但研究本身无伦理风险。
这些应交由后续评审作为修改意见，而非桌面拒稿。本任务是投稿前自检，请从宽把关，避免误拒。
